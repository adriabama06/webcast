...
module.exports = class extends EventEmitter {
    ...
    async main(input, output) {
        ...
        await this.__sleep(4000);
        this.emit('start', this.frames);
        // Nvidia
        // ffmpeg -i ${input} -map 0:0 -map 0:1 -acodec mp3 -vcodec h264_nvenc -profile high -preset medium ${output}
        child_process.exec(`ffmpeg -i "${input}" -map 0:0 -map 0:1 -acodec mp3 -vcodec libx264 -preset veryfast -profile main -threads 4 -b:a 160k -b:v 6M "${output}"`).stderr.on('data', async (data) => {
            const args = data.toString().split('=');
            if(!args || !args.length) {
                return;
            };
